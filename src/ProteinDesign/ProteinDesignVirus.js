...
const ProteinDesignVirus = ({ data }) => {
  const [indexPrices, setIndexPrices] = useState(0);
  const [circleBtns, setCircleBtns] = useState(0);
  const [pkgNames, setPkgNames] = useState(0);
  const [cartListed, setCartListed] = useState([]);

  useEffect(() => {
    setCircleBtns(JSON.parse(window.localStorage.getItem("circleBtns")));
  }, []);
  useEffect(() => {
    window.localStorage.setItem("circleBtns", circleBtns);
  }, [circleBtns]);

  useEffect(() => {
    setIndexPrices(JSON.parse(window.localStorage.getItem("indexPrices")));
  }, []);
  useEffect(() => {
    window.localStorage.setItem("indexPrices", indexPrices);
  }, [indexPrices]);

  useEffect(() => {
    setPkgNames(JSON.parse(window.localStorage.getItem("pkgNames")));
  }, []);
  useEffect(() => {
    window.localStorage.setItem("pkgNames", pkgNames);
  }, [pkgNames]);
      ...
      <section className="pdesing-section">
        <div>
          <div className="pdesign-protein-title">
            ...
            {pkgNames === 0 && <p>Researcher Packages</p>}

            {pkgNames === 1 && <p>Designer Packages</p>}

            {pkgNames === 2 && <p>Leader Packages</p>}
          </div>
